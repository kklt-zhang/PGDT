...
class SoftTeacher(SemiBaseDetector):
    ...
    def rcnn_cls_loss_by_pseudo_instances(self, x: Tuple[Tensor],
        ...
        # ✅ 防止伪标签为空时 KeyError: 'loss_cls'
        if 'loss_cls' in losses:
            # cls_reg_targets[1] 是 label_weights
            losses['loss_cls'] = losses['loss_cls'] * len(cls_reg_targets[1]) / max(sum(cls_reg_targets[1]), 1.0)
        else:
            #import torch
            # 返回一个 0 loss，保证梯度计算不出错
            losses['loss_cls'] = torch.tensor(
                0.0, device=bbox_results['cls_score'].device, requires_grad=True
            )
